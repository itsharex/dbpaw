...
pub fn mysql_form_from_test_context<'a>(
    ...
    if should_reuse_local_db() {
        return (None, mysql_form_from_local_env());
    }
    ...
    let runnable =
        RunnableImage::from(image).with_container_name(shared::unique_container_name("mysql"));
    ...
    apply_mysql_env_overrides(&mut form);
    (Some(container), form)
}

fn mysql_form_from_local_env() -> ConnectionForm {
    let mut form = ConnectionForm {
        driver: "mysql".to_string(),
        host: Some(shared::env_or("MYSQL_HOST", "localhost")),
        port: Some(shared::env_i64("MYSQL_PORT", 3306)),
...
fn apply_mysql_env_overrides(form: &mut ConnectionForm) {
    ...
    if let Ok(port) = env::var("MYSQL_PORT") {
        form.port = Some(
            port.parse::<i64>()
                .expect("MYSQL_PORT should be a valid number"),
        );
    }
