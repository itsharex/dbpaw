...
async fn wait_until_doris_ready(form: &ConnectionForm) {
    ...
    panic!("doris is not ready for command tests: {last_error}");
}

async fn init_state_with_local_db() -> AppState {
    let state = AppState::new();
    let local_db_dir = std::env::temp_dir().join(unique_name("dbpaw_doris_stateful_it"));
...
async fn create_doris_connection_for_state(state: &AppState, base_form: &ConnectionForm) -> i64 {
    let mut form = base_form.clone();
    form.name = Some(unique_name("doris-command"));
...
async fn test_doris_command_create_database_by_id_success() {
    let docker = (!doris_context::should_reuse_local_db()).then(Cli::default);
    let (_container, form) = doris_context::doris_form_from_test_context(docker.as_ref());
    wait_until_doris_ready(&form).await;

    let state = init_state_with_local_db().await;
    let conn_id = create_doris_connection_for_state(&state, &form).await;
