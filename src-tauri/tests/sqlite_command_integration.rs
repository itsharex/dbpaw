...
fn sqlite_test_path() -> PathBuf {
    ...
    p.push(format!("dbpaw-sqlite-cmd-{}.db", Uuid::new_v4()));
    p
}
...
async fn cleanup_table(form: &ConnectionForm, table: &str) {
    let driver = SqliteDriver::connect(form)
        .await
        .expect("failed to connect sqlite driver for cleanup");
    driver
        .execute_query(format!("DROP TABLE IF EXISTS {}", table))
        .await
        .ok();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_test_connection_success() {
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_test_connection_invalid_file_path_returns_error() {
    let form = ConnectionForm {
        driver: "sqlite".to_string(),
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_list_tables_by_conn_contains_created_table() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_list_databases_contains_main() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_execute_by_conn_select_returns_rows() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_execute_by_conn_invalid_sql_returns_error() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_execute_by_conn_insert_affects_rows() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_get_table_data_by_conn_pagination_works() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
...
#[tokio::test]
#[ignore]
async fn test_sqlite_command_get_table_data_by_conn_invalid_pagination_returns_error() {
    let db_path = sqlite_test_path();
    let db_path_str = db_path.to_string_lossy().to_string();
