...
async fn create_mysql_connection_for_state(
    state: &AppState,
    base_form: &ConnectionForm,
    suffix: &str,
) -> i64 {
    let mut form = base_form.clone();
    form.name = Some(format!("mysql-stateful-{suffix}"));
...
async fn prepare_metadata_fixture(
    ...
    let driver = MysqlDriver::connect(form)
        .await
        .expect("failed to connect mysql driver for metadata fixture");
...
#[tokio::test]
#[ignore]
async fn test_mysql_command_list_databases_by_id_invalid_id_returns_error() {
    let state = init_state_with_local_db().await;
...
async fn test_mysql_command_connection_crud_flow_create_get_update_delete() {
    ...
    create_form.name = Some(format!("mysql-{unique}-created"));
    ...
    update_form.name = Some(format!("mysql-{unique}-updated"));
    ...
    assert_eq!(updated.name, format!("mysql-{unique}-updated"));
...
#[tokio::test]
#[ignore]
async fn test_mysql_command_storage_saved_query_crud_flow() {
    let state = init_state_with_local_db().await;
...
async fn test_mysql_command_transfer_export_and_import_minimal_flow() {
    ...
    let cleanup_driver = MysqlDriver::connect(&form)
        .await
        .expect("failed to connect mysql driver for transfer cleanup");
